...
def save_combined_pdf(df_numeric, wordcloud=None, word_freq=None,
                      include_wealth=True, include_wordcloud=True, include_topwords=True, include_recommend=True, include_sources=True):
    pdf = FPDF()
    pdf.add_page()
    ...
    if include_recommend:
        pdf.add_page()
        pdf.set_font("Arial", "B", 14)
        pdf.cell(0, 10, "Insights & Recommendation", ln=True)
        pdf.set_font("Arial", "", 12)
        pdf.multi_cell(0, 10,
            "- Buy if you want long-term stability and property appreciation.\n"
            "- Rent if you value flexibility and liquidity.\n"
            "- Based on Malaysian blogs: consider your savings, career mobility, and family plans before deciding."
        )
    ...
    if include_sources:
        ...
        pdf.set_font("Arial", "", 12)
        pdf.multi_cell(0, 10,
            "1. Great Eastern Life: Should you buy or rent a property in Malaysia?\n"
            "2. KWSP: Buy vs Rent Malaysia\n"
            "3. iProperty: Should you buy or rent property in Malaysia?"
        )
...
# Combined Insights
    ...
    # Show insights live in Streamlit
    ...
    if include_wordcloud and "wordcloud" in locals():
        st.subheader("☁️ WordCloud")
        wc_buf = get_wordcloud_image(wordcloud)
        st.image(wc_buf)

    if include_topwords and "word_freq" in locals():
        st.subheader("📊 Top Words Frequency")
        st.write(pd.DataFrame(word_freq, columns=["Word", "Frequency"]))

    st.subheader("💡 Insights & Recommendation")
    st.info("""
- **Buy** if you want long-term stability and property appreciation.  
- **Rent** if you value flexibility and liquidity.  
- Based on Malaysian blogs: consider your savings, career mobility, and family plans before deciding.
""")
    ...
    st.markdown("""
1. [Great Eastern Life](https://www.greateasternlife.com/my/en/personal-insurance/greatpedia/live-great-reads/wellbeing-and-success/should-you-buy-or-rent-a-property-in-malaysia.html)  
2. [KWSP](https://www.kwsp.gov.my/en/w/article/buy-vs-rent-malaysia)  
3. [iProperty](https://www.iproperty.com.my/guides/should-buy-or-rent-property-malaysia-30437)  
    """)
